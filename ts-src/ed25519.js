...
function xpt_double(pt1) {
    ...
    var J = x1.plus(y1).mod(q);
    var E = J.times(J).minus(A).minus(B).mod(q);
    var G = D.plus(B).mod(q);
    var F = G.minus(C).mod(q);
    var H = D.minus(B).mod(q);
    var X3 = E.times(F).mod(q);
    var Y3 = G.times(H).mod(q);
    var Z3 = F.times(G).mod(q);
    var T3 = E.times(H).mod(q);
    return [X3, Y3, Z3, T3];
}
